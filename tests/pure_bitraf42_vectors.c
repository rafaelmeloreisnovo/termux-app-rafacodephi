...
static int check_u64(rafz_u64 actual, rafz_u64 expected) {
...
int main(void) {
    int failures = 0;
    const rafz_u64 pure = rafz_pure_bitraf42_encode(1u, 2u, 3u, 4u, 5u);
    const rafz_u64 legacy = (rafz_u64)bitraf_encode(1u, 2u, 3u, 4u, 5u);
    const rafz_u64 maxword =
        rafz_pure_bitraf42_encode(0xFFu, 0xFFu, 0xFFFFu, 0xFFFFu, 0xFFFFu);

    failures += check_u64(pure, (rafz_u64)0x0000001401802005ULL);
    ...
    failures += check_u32(
        rafz_pure_bitraf42_is_valid(maxword | ((rafz_u64)1u << 42u)),
        0u);
