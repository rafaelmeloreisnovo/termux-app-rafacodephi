...
class SevenGuardTests(unittest.TestCase):
    ...
    def test_github_ref_must_be_exact_commit(self):
        r = self.run_unit(lambda u: u["provenance"].__setitem__("ref","master"))
    ...
    def test_open_contradiction_blocks(self):
        def m(u): u["contradictions"]=[{"id":"C1","state":"OPEN","comparison_scope":"same APK","ref":"fixture:C1"}]
    ...
    def test_uncertainty_blocks(self):
        def m(u): u["uncertainty"]=[{"id":"U1","state":"TOKEN_VAZIO","evidence_needed":"device","falsifier":"receipt","next_probe":"run device gate"}]
