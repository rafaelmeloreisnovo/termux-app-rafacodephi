...
class SevenGuardTests(unittest.TestCase):
    ...
    def test_open_contradiction_blocks(self):
        def m(u):
            u["contradictions"] = [{"id":"C1","state":"OPEN","comparison_scope":"same APK","ref":"fixture:C1"}]
        self.assertTrue(any("OPEN" in x for x in self.run_unit(m)["blockers"]))

    def test_uncertainty_blocks(self):
        def m(u):
            u["uncertainty"] = [{"id":"U1","state":"TOKEN_VAZIO","evidence_needed":"device","falsifier":"receipt","next_probe":"run device gate"}]
