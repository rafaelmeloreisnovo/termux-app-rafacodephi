...
class SevenGuardTests(unittest.TestCase):
    ...
    def test_real_handoff_is_structurally_valid_but_blocked(self):
        ...
        self.assertIn("contradiction:C-APP-PROVIDER-RULESET-001:OPEN", result["blockers"])
        self.assertIn("uncertainty:U-APP-PHYSICAL-RUNTIME-001:TOKEN_VAZIO", result["blockers"])
    ...
    def test_open_contradiction_blocks(self):
        def mutate(unit):
            unit["contradictions"] = [
                {"id": "C1", "state": "OPEN", "comparison_scope": "same APK", "ref": "fixture:C1"}
            ]
    ...
    def test_resolved_contradiction_requires_resolution(self):
        def mutate(unit):
            unit["contradictions"] = [
                {"id": "C1", "state": "RESOLVED", "comparison_scope": "same APK", "ref": "fixture:C1"}
            ]
    ...
    def test_invalid_contradiction_state_fails_structure(self):
        def mutate(unit):
            unit["contradictions"] = [
                {"id": "C1", "state": "MAYBE", "comparison_scope": "same APK", "ref": "fixture:C1"}
            ]
    ...
    def test_invalid_contradiction_state_rejected(self):
        def m(u): u["contradictions"]=[{"id":"C1","state":"IGNORED","comparison_scope":"same APK","ref":"fixture:C1"}]
            ...
            unit["uncertainty"] = [
                {
                    "id": "U1",
                    "state": "TOKEN_VAZIO",
                    "evidence_needed": "device",
                    "falsifier": "receipt",
                    "next_probe": "run device gate",
                }
            ]
    ...
    def test_invalid_uncertainty_state_rejected(self):
        def m(u): u["uncertainty"]=[{"id":"U1","state":"MAGIC","evidence_needed":"device","falsifier":"receipt","next_probe":"run device gate"}]
