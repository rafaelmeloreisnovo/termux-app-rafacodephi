 ...
 * It only asks Android's linker to execute the packaged ELF and renders stdout.
 */
public final class BenchmarkMenuActivity extends Activity {
    ...
    private android.view.View layout() {
        ...
        output.setText("ELF not executed.");
    ...
    private void execute() {
        ...
        output.setText("Executing direct ELF entry…");

        new Thread(() -> {
            String result;
            try {
                File elf = new File(getApplicationInfo().nativeLibraryDir, "libraf_pa_core.so");
                if (!elf.isFile()) throw new IllegalStateException("ELF missing: " + elf);

                String linker = selectLinker();
