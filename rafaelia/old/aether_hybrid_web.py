...
Licença sugerida: MIT (pode trocar depois).
"""
...
def aether_update(state, chunk: bytes):
    """Atualiza um estado de 64 bits com um chunk (igual espírito do C)."""
    h = state
    data = chunk
    n_blocks = len(data) // 8
...
def aether_poly_hash_stream(stream, width_bits: int, chunk_size: int = 4 * 1024 * 1024):
    """Multi-lane AETHER (64–1024 bits) em streaming (tipo BLAKE)."""
...
def open_as_stream_from_request():
    ...
    - arquivo (file)
    - ou texto (text)
    Retorna um objeto tipo stream (BytesIO) e um label.
    """
...
def parse_engine_and_width():
    engine = (request.form.get("engine")
              or (request.json.get("engine") if request.is_json else None)
              or "aether").lower()
    width_str = (request.form.get("width")
                 or (request.json.get("width") if request.is_json else None)
                 or "256")
    try:
        width = int(width_str)
    except ValueError:
...
if __name__ == "__main__":
    ...
    # host="0.0.0.0" para acessar de outros dispositivos na rede local
