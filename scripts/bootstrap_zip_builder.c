...
int main(int argc,char**argv){
  ...
  static const uint8_t sh[]="#!/system/bin/sh\necho RAFCODEPHI bootstrap shell\n";
  static const uint8_t pkg[]="#!/system/bin/sh\necho RAFCODEPHI pkg placeholder\n";
  E e[3]={{"BOOTSTRAP_INFO",(uint8_t*)info,(uint32_t)info_n,0,0},{"bin/sh",sh,sizeof(sh)-1,0,0},{"bin/pkg",pkg,sizeof(pkg)-1,0,0}};
  crc32_init(); for(int i=0;i<3;i++) e[i].crc=crc32_calc(e[i].data,e[i].size);
  ...
  uint8_t z[22]; memset(z,0,sizeof(z)); le32(z,0x06054b50); le16(z+8,3); le16(z+10,3); le32(z+12,cdir_sz); le32(z+16,cdir_off);
