...
rg -q 'prepare_bootstrap_env\.sh' scripts/build_release_artifacts.sh || fail "build_release_artifacts.sh must call prepare_bootstrap_env.sh"
rg -q 'prepare_bootstrap_env\.sh' scripts/build_apk_matrix.sh || fail "build_apk_matrix.sh must call prepare_bootstrap_env.sh"

rg -q '🧬 Prepare Bootstrap Environment' .github/workflows/rafaelia_pipeline.yml || fail "workflow must contain standardized bootstrap step"
rg -q 'prepare_bootstrap_env\.sh --github-env' .github/workflows/rafaelia_pipeline.yml || fail "workflow must call prepare_bootstrap_env.sh --github-env"

if rg -n 'LOCAL_CFLAGS.*(fno-rtti|fno-exceptions)|fno-rtti|fno-exceptions' app/src/main/cpp/Android.mk | rg 'LOCAL_CFLAGS'; then
