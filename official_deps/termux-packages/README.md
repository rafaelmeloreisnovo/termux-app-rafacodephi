...
## RAFAELIA low-level boosters

This fork adds **rafpkg** (`raf_pkg/`) — a deterministic, static parser in C that can:

- scan package roots → JSON index
- build a dependency graph → deterministic build order
- probe CPU features → select faster kernels
- audit trees for "soltos" / unknown blobs (ELF/scripts/no-ext) + RafStore cache (less IO)

See: `raf_pkg/README.md`.
