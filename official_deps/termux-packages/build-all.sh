...
if [ -e "$BUILDORDER_FILE" ]; then
	...
	if [ "${RAFPKG:-}" = "1" ] || [ "${RAFPKG:-}" = "true" ]; then
		echo "Using rafpkg to generate buildorder..."
		"$TERMUX_SCRIPTDIR/scripts/rafpkg.sh" plan --arch "$TERMUX_ARCH" > "$BUILDORDER_FILE"
	else
...
# Fast membership test: load built package names once (avoid per-iteration grep)
declare -A BUILT_SET
if [ -e "$BUILDSTATUS_FILE" ]; then
	while read -r _p; do
		[ -n "$_p" ] && BUILT_SET["$_p"]=1
	done < "$BUILDSTATUS_FILE"
fi
...
# Build status snapshot (keep for fast resume + audit)
TS=$(date "+%Y%m%d_%H%M%S")
mkdir -p ".rafstore"
cp -f "$BUILDSTATUS_FILE" ".rafstore/buildstatus.${TS}.txt" 2>/dev/null || true
cp -f "$BUILDSTATUS_FILE" ".rafstore/buildstatus.last.txt" 2>/dev/null || true
echo "Finished (buildstatus kept; snapshot in .rafstore)"
